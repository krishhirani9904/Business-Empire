        ...
        <div className={`${c.bg} w-full sm:max-w-md sm:rounded-2xl rounded-t-3xl max-h-[85vh] overflow-y-auto`}>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className={`text-lg font-bold ${c.text}`}>{stock.name}</h3>
                <p className={`text-xs ${c.textSec}`}>{stock.fullName}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className={`p-3 rounded-xl ${c.inner}`}>
                ...
                <p className={`text-sm font-semibold ${c.text}`}>{stock.sector}</p>
              </div>
              <div className={`p-3 rounded-xl ${c.inner}`}>
              ...
              <div className={`p-3 rounded-xl ${c.inner}`}>
                ...
                <p className={`text-sm font-semibold ${c.text}`}>{formatMarketCap(stock.capitalization)}</p>
              </div>
              <div className={`p-3 rounded-xl ${c.inner}`}>
                <p className={`text-[10px] ${isDarkTheme ? 'text-gray-500' : 'text-gray-400'}`}>Volatility</p>
                <p className={`text-sm font-semibold ${
                  stock.volatility > 7 ? 'text-red-500' : stock.volatility > 4 ? 'text-yellow-500' : 'text-green-500'
                }`}>
            ...
            {owned && (
              <div className={`p-3 rounded-xl border ${isDarkTheme ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'}`}>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    <p className={`text-[10px] ${c.textSec}`}>Shares</p>
