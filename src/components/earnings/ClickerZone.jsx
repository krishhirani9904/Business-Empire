...
const ClickerZone = ({ onClick, isBoosted, perClick }) => {
  ...
  const handleClick = (e) => {
    ...
    // Add floating text with controlled position
  ...
  return (
    <div className={`${c.containerBg} rounded-xl sm:rounded-2xl p-4 sm:p-6 border ${c.containerBorder} transition-colors duration-300`}>
      <div className="flex flex-col items-center justify-center">
        
        {/* Main Button */}
        <div className="relative">
          
          {/* Floating Rupees Text */}
          {floatingTexts.map(ft => (
            <div
              key={ft.id}
              className={`absolute pointer-events-none z-20 font-bold text-base sm:text-lg whitespace-nowrap
