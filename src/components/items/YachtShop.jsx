...
import { X, ShoppingCart, ChevronRight, Check, MapPin } from 'lucide-react';
...
function YachtShop() {
  ...
  const calculatePrice = (yacht) => {
    ...
    if (hireTeam && yacht.hireTeam) price *= yacht.hireTeam.priceMultiplier;
  ...
  const handleSelect = (yacht) => {
    ...
    setHireTeam(false);
    setSelectedLocation(yacht.locations?.[0] || null);
  ...
  const handleBuy = () => {
    ...
    const success = buyItem('Yachts', selected, totalPrice, {
      selectedDesign,
      hasTeam: hireTeam,
      selectedLocation,
      type: selected.type
        ...
        <div className={`fixed inset-0 ${c.overlay} z-[100] flex items-end sm:items-center justify-center`}>
          <div className={`${isDarkTheme ? 'bg-gray-900' : 'bg-white'} w-full sm:max-w-md sm:rounded-2xl rounded-t-3xl max-h-[85vh] overflow-y-auto`}>
            ...
            <div className="p-4 space-y-4">
              ...
              {/* Design */}
              <div>
                <h4 className={`text-sm font-bold ${c.text} mb-2`}>🎨 Deck Design</h4>
                <div className="grid grid-cols-2 gap-2">
                  {selected.designLevels.map(design => (
                    <button key={design.id} onClick={() => setSelectedDesign(design)}
                      ...
                      {design.priceMultiplier > 1 && <p className="text-yellow-500 text-[10px] mt-0.5">+{Math.round((design.priceMultiplier - 1) * 100)}%</p>}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <button onClick={() => setHireTeam(!hireTeam)}
                  ...
                  <div>
                    ...
                    <p className="text-yellow-500 text-[10px] mt-0.5">+{Math.round((selected.hireTeam.priceMultiplier - 1) * 100)}%</p>
                  </div>
                  <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center ${hireTeam ? 'bg-cyan-500 border-cyan-500' : c.innerBorder}`}>
              ...
              {selected.locations && (
                <div>
                  <h4 className={`text-sm font-bold ${c.text} mb-2`}>📍 Dock Location</h4>
                  <div className="grid grid-cols-2 gap-2">
                    {selected.locations.map(loc => (
                      <button key={loc} onClick={() => setSelectedLocation(loc)}
                        className={`p-2.5 rounded-xl border text-left transition-all text-xs flex items-center gap-1.5
                          ${selectedLocation === loc ? c.selected : `${c.innerBg} border ${c.innerBorder}`}`}>
                        <MapPin className="w-3 h-3" />
                        <span className={`font-medium ${c.text} text-[11px]`}>{loc}</span>
                      </button>
                    ))}
                  </div>
                </div>
