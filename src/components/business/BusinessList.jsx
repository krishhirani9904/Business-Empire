// components/business/BusinessList.jsx
import React from 'react';
import { useTheme } from '../../context/ThemeContext';
import { BUSINESSES, formatCurrency } from './businessData';

function BusinessList({ ownedBusinesses, onSelectBusiness }) {
        ...
        {BUSINESSES.map(business => {
          ...
          return (
            ...
            >
              {/* Owned Badge */}
              {ownedCount > 0 && (
                <div className="absolute top-2 right-2 bg-green-500 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center font-bold">
                  {ownedCount}
                </div>
              )}
              
              {/* Icon */}
              <div className={`w-10 h-10 bg-gradient-to-r ${business.color} rounded-lg flex items-center justify-center mb-2`}>
